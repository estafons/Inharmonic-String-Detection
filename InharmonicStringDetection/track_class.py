import constants, Inharmonic_Detector

class Tablature():
    def __init__(self, tups, audio, annotation = False):
        self.tablature = []
        note_audio = []
        for x in tups:
            if annotation == False:
            ...
            self.tablature.append(TabInstance(x, note_audio))
...
class TrackInstance():
    def __init__(self, tablature : Tablature, audio): # tup gets the onset -fundamental tuple
...
class Annotations():
    def __init__(self, tups):
        self.tablature = Tablature(tups, [], annotation = True)
